...
module.exports.execute = async(client, message, args,ayar,emoji) => {
   ...
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    const embed = new MessageEmbed().setColor("2F3136").setAuthor(kullanici.tag.replace('`', '')+` ( ` + message.author.id + ` )` , kullanici.avatarURL({dynamic: true, size: 2048})).setThumbnail(kullanici.avatarURL({dynamic: true, size: 2048}));
    MemberStats.findOne({ guildID: message.guild.id, userID: uye.id }, (err, data) => {
        ...
        embed.addField('Haftalık Ses',`○ ** ${client.convertDuration(haftalikSesToplam)}** \n ${haftalikSesListe}`);
        embed.addField('Haftalık Chat',`○  ** ${haftalikChatToplam} mesaj** \n ${haftalikChatListe}`);
