module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if(args[0] === 'tüm') {
        ...
        message.member.voice.channel.members.array().forEach((x, index) => setTimeout(() => { x.voice.setChannel(argskanal.id); }, index*1000));
      ...
      return  message.channel.send( kanalsıra.map((x, index) => `${index+1}-) ${x.name}`).join('\n') + '\n\n30 saniye içinde, kanalındaki kişilerin taşınacağı ses kanalının numarasını girmelisin!', {code: 'css', split: true});
        try {
            var filter = m => m.author.id === message.author.id && Number(m.content) < kanallar.size;
            message.channel.awaitMessages(filter, {max: 1, time: 30000, errors: ['time']}).then((collected) => {
                if(isNaN(collected.first().content)) return message.reply(`**Geçerli bir ses kanalı numarası belirtmelisin!**`).then(x => x.delete({timeout: 5000}));
            ...
            return message.channel.send( '30 saniye içinde oda belirtmediğiniz için işlem iptal edilmiştir!').then(x => x.delete({ timeout: 10000 }));
        }
    }
};
